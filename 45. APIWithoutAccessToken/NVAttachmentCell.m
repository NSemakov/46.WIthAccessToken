    ...
    if (self) {
        ...
        for (NSDictionary* obj in att) {
            if ([[obj objectForKey:@"type"] isEqualToString:@"audio"]) {
                ...
                NVPhoto* photo=[[NVPhoto alloc]initWithDictionary:obj];
                [self.audios addObject:photo];
    ...
    if ([self.photos count]) {
        ...
        for (NSInteger i=0; i<[arrayOfPhotos count];i++) {
            ...
            for (NSInteger j=0; j<[row count]; j++) {
                ...
                CGRect rect;
                if (photo.isVertical) {
                    CGFloat widthResized=130.f*([photo.widthPhoto floatValue]/[photo.heightPhoto floatValue]);
                    rect=CGRectMake(self.lastFrame.origin.x+CGRectGetWidth(self.lastFrame)+5,  self.lastFrame.origin.y+5+130, widthResized, 130);
                } else {
                    CGFloat heightResized=130.f/([photo.widthPhoto floatValue]/[photo.heightPhoto floatValue]);
...
- (void) resizePhotos:(NSArray*) arrayOfPhoto {
    for (NVPhoto* obj in arrayOfPhoto){
        CGFloat ratio=[[obj widthPhoto] floatValue]/[[obj heightPhoto] floatValue];
